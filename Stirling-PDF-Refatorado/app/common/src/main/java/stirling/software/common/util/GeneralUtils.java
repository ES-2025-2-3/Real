...
public class GeneralUtils {
    ...
    public Long convertSizeToBytes(String sizeStr, String defaultUnit) {
        ...
        String normalizedSize =
                sizeStr.trim().toUpperCase(Locale.ROOT).replace(",", ".").replace(" ", "");

        try {
            if (normalizedSize.endsWith("TB")) {
                return toBytes(
                        parseSizeValue(normalizedSize.substring(0, normalizedSize.length() - 2)),
                        4);
                ...
                return toBytes(
                        parseSizeValue(normalizedSize.substring(0, normalizedSize.length() - 2)),
                        2);
            } else if (normalizedSize.endsWith("KB")) {
                return toBytes(
                        parseSizeValue(normalizedSize.substring(0, normalizedSize.length() - 2)),
                        1);
            } else if (!normalizedSize.isEmpty()
                    && normalizedSize.charAt(normalizedSize.length() - 1) == 'B') {

                return toBytes(
                        parseSizeValue(normalizedSize.substring(0, normalizedSize.length() - 1)),
                        0);
            } else {

                String unit = defaultUnit != null ? defaultUnit.toUpperCase(Locale.ROOT) : "MB";

                BigDecimal value = parseSizeValue(normalizedSize);
                ...
                return switch (unit) {
                    ...
                    default -> toBytes(value, 2);
                };
            }
        } catch (NumberFormatException e) {
            log.warn("Failed to parse size string '{}': {}", normalizedSize, e.getMessage());
            return null;
        }
    }
